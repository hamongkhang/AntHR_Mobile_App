...
const DeliveringScreen = ({ navigation }) => {
    ...
    const getOrders = (token) => {
        setLoading(true);
        fetch(REACT_APP_API + '/cart_present/getAllCartPresent', {
            method: "GET",
    ...
    const onConfirmEmployee = (id) => {
        setLoading(true);
        fetch(REACT_APP_API + '/cart_present/changeStatusClient/' + id, {
            method: "GET",
            headers: { "Authorization": `Bearer ` + token }
        })
            .then(response => response.json())
                                    ...
                                    return (
                                        <View style={{ marginBottom: 10, padding: 10, backgroundColor: "white", borderColor: "rgb(227, 235, 241)", borderWidth: 1, borderRadius: 5, paddingBottom: 16, boxShadow: "rgb(95 125 149 / 20%) 0px 4px 13px 0px" }}>
                                                ...
                                                <View style={{ width: "20%" }}>
                                                    {item.avatar ?
                                                        (item.avatar.search('https://') !== -1) ?
                                                            ...
                                                            :
                                                            <Avatar.Image size={45} style={{ backgroundColor: "#edf8f1" }} source={{ uri: REACT_APP_FILE + '/avatar/' + item.avatar }} />
                                                        :
                                            ...
                                            <View style={{ alignItems: "center", marginBottom: 20 }}>
                                                <Image
                                                    source={{ uri: REACT_APP_FILE + '/present/image/' + item.present_image }}
                                                    style={{
                                                        width: '100%',
                                                        height: 100,
                                                        resizeMode: 'contain',
